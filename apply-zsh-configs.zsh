...
apply_zsh_configs() {
    ...
    then
        ...
        BACKUP_FOLDER_NAME=".zsh-configs_$(date +'%F_%H-%M-%S')"
        mkdir -p $HOME/$BACKUP_FOLDER_NAME

        cp -r $HOME/.zsh-configs/* $HOME/$BACKUP_FOLDER_NAME/

        echo
        echo "> Backed up the existing contents to \"$HOME/$BACKUP_FOLDER_NAME\" folder"
